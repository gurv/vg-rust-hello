...
export function getItemFilePath(data: ExplorerItem) {
	if (data.type === "Path" || data.type === "NonIndexedPath")
		return data.item;
...
export function isPath(
	item: ExplorerItem,
): item is Extract<ExplorerItem, { type: "Path" }> {
...
export function insertLibrary(
	queryClient: QueryClient,
	library: LibraryConfigWrapped,
) {
	queryClient.setQueryData(["library.list"], (libraries: any) => {
		// The invalidation system beat us to it
		if (libraries.items.find((l: any) => l.__id === library.uuid))
			return libraries;
