...
export function getTotalTasks(jobs: JobReport[]) {
	...
	jobs?.forEach(
		({ task_count, status, completed_at, completed_task_count }) => {
			tasks.total += task_count;
			tasks.completed +=
				status === "Completed" ? task_count : completed_task_count;
			if (status === "Completed") {
...
export function getJobNiceActionName(
	action: string,
	completed: boolean,
	job?: JobReport,
) {
