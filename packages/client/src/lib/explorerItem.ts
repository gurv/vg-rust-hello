...
export function getExplorerItemData(data?: ExplorerItem | null): ItemData {
	...
	switch (data.type) {
		...
		case "Path": {
			...
			if (object?.kind)
				itemData.kind = ObjectKind[object?.kind] ?? "Unknown";
			...
			if (filePath) {
				...
				itemData.extension =
					filePath.extension?.toLocaleLowerCase() ?? null;
				//
				if ("cas_id" in filePath) itemData.casId = filePath.cas_id;
				if ("location_id" in filePath)
					itemData.locationId = filePath.location_id;
				if ("date_indexed" in filePath)
					itemData.dateIndexed = filePath.date_indexed;
				if ("date_modified" in filePath)
					itemData.dateModified = filePath.date_modified;
			}
			break;
		}
		...
		case "Location": {
			...
			if (location) {
				if (
					location.total_capacity != null &&
					location.available_capacity != null
				)
					itemData.size = byteSize(
